...
WITH waivers AS
  ( SELECT DISTINCT bpr.customer_id AS member_id,
   ...
   WHERE bpr.billing_product::text = 'FEES.PER_TRIP_WAIVER'::text
     AND bpr.net > 0::double precision ),
     credits AS
  ( SELECT customer_credit.account_id AS member_id,
   ...
   WHERE customer_credit.end_date_utc > now()
     AND customer_credit.available_amount > 0::double precision
   GROUP BY customer_credit.account_id ),
     bookings AS
  ( SELECT DISTINCT first_value(bo.member_id) OVER (PARTITION BY bo.member_id
                                                    ORDER BY bo.booking_start_date) AS member_id,
                                                   first_value(bo.booking_start_date) OVER (PARTITION BY bo.member_id
                                                                                            ORDER BY bo.booking_start_date) AS first_res_date,
                                                                                           first_value(initcap("left"(v.name::text, "position"(v.name::text, ' '::text) - 1))) OVER (PARTITION BY bo.member_id
                                                                                                                                                                                     ORDER BY bo.booking_start_date) AS first_res_vehicle_name,
                                                                                                                                                                                    first_value(bo.start_zone_name) OVER (PARTITION BY bo.member_id
                                                                                                                                                                                                                          ORDER BY bo.booking_start_date) AS first_res_start_zone,
                                                                                                                                                                                                                         first_value(bo.booking_start_date) OVER (PARTITION BY bo.member_id
                                                                                                                                                                                                                                                                  ORDER BY bo.booking_start_date DESC) AS last_res_date,
                                                                                                                                                                                                                                                                 first_value(initcap("left"(v.name::text, "position"(v.name::text, ' '::text) - 1))) OVER (PARTITION BY bo.member_id
                                                                                                                                                                                                                                                                                                                                                           ORDER BY bo.booking_start_date DESC) AS last_res_vehicle_name,
                                                                                                                                                                                                                                                                                                                                                          first_value(bo.start_zone_name) OVER (PARTITION BY bo.member_id
                                                                                                                                                                                                                                                                                                                                                                                                ORDER BY bo.booking_start_date DESC) AS last_res_start_zone
...
SELECT DISTINCT mem.member_id ||'_zipcar' AS subscriber_key,
                ...
                CASE
                    WHEN (mem.birth_date + (((date_part('year'::text, 'now'::text::date)::integer - date_part('year'::text, mem.birth_date)::integer) || ' years'::text)::interval)) >= 'now'::text::date
                ...
                CASE
                    WHEN mem.email_consent_p = 't' THEN TRUE::bool
                    WHEN mem.email_consent_p = 'f' THEN FALSE::bool
                END AS is_emailable,
                CASE
                    WHEN mem.role::text = 'customer'::text
                         AND mem.workflow_status::text = 'Approved'::text THEN TRUE::bool
                    ELSE FALSE::bool
                END AS is_current_approved_member,
                CASE
                    WHEN (mem.role::text = ANY (ARRAY['staff'::text,
                                                      'admin'::text]))
                         AND mem.workflow_status::text = 'Approved'::text THEN TRUE::bool
                    ELSE FALSE::bool
                END AS is_employee,
                CASE
                    WHEN mem.role::text = 'customer'::text
                         AND (mem.workflow_status::text = ANY (ARRAY['Pending'::text,
                                                                     'In review'::text])) THEN TRUE::bool
                    ELSE FALSE::bool
                END AS is_prospect,
                mem.role,
                CASE
                    WHEN z4b.member_id IS NOT NULL THEN TRUE::bool
                    ELSE FALSE::bool
                ...
                CASE
                    WHEN mem.credit_card IS NOT NULL
                         AND mem.credit_card_expiry > now() THEN TRUE::bool
                    ELSE FALSE::bool
