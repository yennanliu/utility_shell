...
CREATE OR REPLACE VIEW prc.v_cc_individuals_brussels AS
 WITH waivers AS (
         SELECT DISTINCT bpr.customer_id AS member_id,
            true AS has_had_any_per_trip_waiver
         ...
         SELECT DISTINCT first_value(bo.member_id) OVER (PARTITION BY bo.member_id ORDER BY bo.booking_start_date) AS member_id,
            first_value(bo.booking_start_date) OVER (PARTITION BY bo.member_id ORDER BY bo.booking_start_date) AS first_res_date,
            first_value(initcap("left"(v.name::text, "position"(v.name::text, ' '::text) - 1))) OVER (PARTITION BY bo.member_id ORDER BY bo.booking_start_date) AS first_res_vehicle_name,
            first_value(bo.start_zone_name) OVER (PARTITION BY bo.member_id ORDER BY bo.booking_start_date) AS first_res_start_zone,
            first_value(bo.booking_start_date) OVER (PARTITION BY bo.member_id ORDER BY bo.booking_start_date DESC) AS last_res_date,
            first_value(initcap("left"(v.name::text, "position"(v.name::text, ' '::text) - 1))) OVER (PARTITION BY bo.member_id ORDER BY bo.booking_start_date DESC) AS last_res_vehicle_name,
            first_value(bo.start_zone_name) OVER (PARTITION BY bo.member_id ORDER BY bo.booking_start_date DESC) AS last_res_start_zone
 ...
 SELECT DISTINCT mem.member_id,
    ...
    date_trunc('month'::text, mem.birth_date::timestamp with time zone) AS birth_month_year,
        CASE
            WHEN (mem.birth_date + (((date_part('year'::text, 'now'::text::date)::integer - date_part('year'::text, mem.birth_date)::integer) || ' years'::text)::interval)) >= 'now'::text::date AND (mem.birth_date + (((date_part('year'::text, 'now'::text::date)::integer - date_part('year'::text, mem.birth_date)::integer) || ' years'::text)::interval)) <= ('now'::text::date + 7) THEN true
            ELSE false
    ...
    mem.email AS email_address,
        ...
        CASE
            WHEN mem.email_consent_p::text = 't'::text THEN true
            WHEN mem.email_consent_p::text = 'f'::text THEN false
            ELSE NULL::boolean
        END AS is_emailable,
        CASE
            WHEN mem.role::text = 'customer'::text AND mem.workflow_status::text = 'Approved'::text THEN true
            ELSE false
        END AS is_current_approved_member,
        CASE
            WHEN (mem.role::text = ANY (ARRAY['staff'::text, 'admin'::text])) AND mem.workflow_status::text = 'Approved'::text THEN true
            ELSE false
        END AS is_employee,
        CASE
            WHEN mem.role::text = 'customer'::text AND (mem.workflow_status::text = ANY (ARRAY['Pending'::text, 'In review'::text])) THEN true
            ELSE false
        END AS is_prospect,
    mem.role,
        CASE
            WHEN z4b.member_id IS NOT NULL THEN true
            ELSE false
    ...
    mem.join_promo AS join_promo_code,
        CASE
            WHEN mem.driving_license IS NULL THEN false
            ELSE true
        END AS has_driving_license,
        CASE
            WHEN mem.credit_card IS NOT NULL AND mem.credit_card_expiry > now() THEN true
            ELSE false
    ...
    COALESCE(w.has_had_any_per_trip_waiver, false) AS has_had_any_per_trip_waiver,
