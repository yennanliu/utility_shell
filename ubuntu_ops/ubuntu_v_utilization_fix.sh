...
psql \
   ...
   --username  $username\
   --dbname=$dbname << EOF
DROP TABLE IF EXISTS rw.vehicle_utilization_fixed;
EOF
...
CREATE TABLE rw.vehicle_utilization_fixed AS
  (WITH dates AS
  (SELECT DISTINCT generate_series(min(vehicle_logs.date) OVER (PARTITION BY vehicle_logs.vin)::TIMESTAMP WITHOUT TIME ZONE, now()::date - '1 day'::interval, '1 day'::interval)::date AS date,
                   ...
                   1 AS capacity_days
   FROM rw.vehicle_logs vehicle_logs
  ...
  (SELECT vehicle_logs.vin,
   ...
   FROM ext_logs vehicle_logs
   WHERE vehicle_logs.category::text ~~ '%Service%'::text
     OR vehicle_logs.category::text = 'Start booking'::text
     OR vehicle_logs.category::text = 'writeoff'::text),
     serv_calcs AS
  (SELECT s_start.vin,
   ...
   AND (s_end.category::text = 'Return to Service'::text
        OR s_end.category::text = 'Start booking'::text
        OR s_end.category::text = 'Out of Service'::text
        OR s_end.category::text = 'writeoff'::text)
   AND s_end.rank = CASE
...
SELECT a.date AS date,
       (a.average_non_operational_fleet) AS average_non_operational_fleet,
       (a.average_operational_fleet) AS average_operational_fleet
FROM rpt_daily_actuals a
WHERE date(date) >= '2017-09-01'
ORDER BY date )
...
psql \
   ...
   --username  $username\
   --dbname=$dbname << EOF
SELECT * FROM  rw.vehicle_utilization_fixed ;
EOF
