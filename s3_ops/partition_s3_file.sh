...
to_bucket=s3://suntory-data/raw_transaciton_data_partition2
...
for s3_file in $s3_files;
    do 
       ...
       echo "copy" $from_bucket/$s3_file "to" $to_bucket/ym=$to_file/$s3_file 
       aws s3 cp $from_bucket/$s3_file $to_bucket/ym=$to_file/$s3_file
    done 